...
class HealthProfessionals extends MY_Controller_CMS {

	public $folder_view = 'health_professionals';
	public $title_module = 'Profissionais da Saúde';
	public $controller_module = 'healthProfessionals';
	...
	public function __construct(){
		...
		$this->table_module = $this->HealthProfessionals_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::healthProfessionals;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->HealthProfessionals_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->HealthProfessionals_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->HealthProfessionals_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key) && !empty($value)){
					$this->HealthProfessionals_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}

			}

			$this->upload_file('health_professionals', $object->id);
	...
	public function upload_file($folder = 'health_professionals', $id = ''){

		if($folder != 'health_professionals') {
			$folder = 'health_professionals/'.$folder;
		}

		if (empty($id)) {

			$item = $this->HealthProfessionals_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->HealthProfessionals_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->HealthProfessionals_model->update(array($file_field => $name_file), array('id' => $id));
