...
class SocialRoom extends MY_Controller_CMS {

	public $folder_view = 'social_romm';
	public $title_module = 'Sala Social';
	public $controller_module = 'socialRoom';
	...
	public function __construct(){
		...
		$this->table_module = $this->SocialRoom_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::socialRoom;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->SocialRoom_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->SocialRoom_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->SocialRoom_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key) && !empty($value)){
					$this->SocialRoom_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}

			}

			$this->upload_file('social_room', $object->id);
	...
	public function upload_file($folder = 'social_room', $id = ''){

		if($folder != 'social_room') {
			$folder = 'social_room/'.$folder;
		}

		if (empty($id)) {

			$item = $this->SocialRoom_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->SocialRoom_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->SocialRoom_model->update(array($file_field => $name_file), array('id' => $id));
