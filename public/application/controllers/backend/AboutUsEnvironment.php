...
class AboutUsEnvironment extends MY_Controller_CMS {

	public $folder_view = 'about_us_environment';
	public $title_module = 'Meio Ambiente';
	public $controller_module = 'aboutUsEnvironment';
	...
	public function __construct(){
		...
		$this->table_module = $this->AboutUsEnvironment_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::aboutUsEnvironment;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->AboutUsEnvironment_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->AboutUsEnvironment_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->AboutUsEnvironment_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key) && !empty($value)){
					$this->AboutUsEnvironment_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}

			}

			$this->upload_file('environment', $object->id);
	...
	public function upload_file($folder = 'environment', $id = ''){
		...
		if (empty($id)) {
			...
			$item = $this->AboutUsEnvironment_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->AboutUsEnvironment_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->AboutUsEnvironment_model->update(array($file_field => $name_file), array('id' => $id));
