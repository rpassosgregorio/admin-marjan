...
class Career extends MY_Controller_CMS {

	public $folder_view = 'career';
	public $title_module = 'Carreira';
	public $controller_module = 'career';
	...
	public function __construct(){
		...
		$this->table_module = $this->Career_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::career;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->Career_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->Career_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->Career_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key) && !empty($value)){
					$this->Career_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}

			}

			$this->upload_file('career', $object->id);
	...
	public function upload_file($folder = 'career', $id = ''){

		if($folder != 'career') {
			$folder = 'career/'.$folder;
		}

		if (empty($id)) {

			$item = $this->Career_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->Career_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->Career_model->update(array($file_field => $name_file), array('id' => $id));
