...
class Controller_Default extends MY_Controller_CMS {

	public $folder_view = 'controller_Default';
	public $title_module = 'Controller_Default';
	public $controller_module = 'controller_Default';
	...
	public function __construct(){
		...
		$this->table_module = $this->Default_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::controller_default;
	...
	public function index($page = 1, $field_search = ''){
		...
		$this->view_data['list_objects'] = $this->Default_model->find(array(), array('deleted = ' => 0), $field_search, array('id'), $order_field, $this->page_list, $this->count_pages_list);
		$this->view_data['total_record'] = $this->Default_model->count(array(), array('deleted = ' => 0), $field_search, array('id'));
	...
	public function insert($page = 1, $field_search = ''){
		...
		$this->view_data['object'] = $this->Default_model;
	...
	public function edit($page = 1, $id, $field_search = ''){
		...
		$this->view_data['page'] = $page;
		$this->view_data['field_search'] = $field_search;
		$this->view_data['object'] = $this->Default_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
		...
		try{
			...
			if(!isset($_FILES) || empty($_FILES)){
				...
				if(!empty($id)){
					...
					$old_object = $this->Default_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
					...
					if($return_upload['result'] == FALSE){
						...
						$this->Default_model->update(array($file_field 		  => $new_name_file,
													   	   'extension' 		  => $extension,
												       	   'date_last_update' => date('Y-m-d H:i:s')),
								 			     	 array('id' => $id));

						$new_object = $this->Default_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
	...
	public function delete_item(){
		...
		$object = $this->Default_model->find_unique(array(), array('id = ' => $this->input->post('id'), 'deleted = ' => 0));
		...
		if($this->is_post()){
			...
			try{
				...
				if($this->is_post()){
					...
					$this->Default_model->update(array('active' => $active,
												       'date_last_update' => date('Y-m-d H:i:s')),
	...
	public function delete(){
		...
		try{
			...
			if(!empty($this->input->post('ids'))){
				...
				foreach($ids as $id){
					...
					$this->Default_model->update(array('deleted' => 1,
												       'date_last_update' => date('Y-m-d H:i:s')),
		...
		echo json_encode(array("result" => $result, "msg" => $result == 'true' ? $feedback_success_message : $feedback_error_message, "total" => $this->Default_model->count(array(), array('deleted = ' => 0), array(), array('id'))));
	...
	private function action_insert(){
		...
		if($this->is_post()){
			...
			try{
				...
				$files = $this->session->tempdata();

				if(!empty($files)){

					foreach($files as $field => $path_full){

						$file_name = '';

						$explode = explode('.', $path_full);
				...
				if($this->form_validation->run() == TRUE){
					...
					$last_id = $this->Default_model->insert(array('name' => $this->input->post('mail'),
																  'inserted_by' => $this->admin_logged->id));


					$new_object = $this->Default_model->find_unique(array(), array('id = ' => $last_id, 'deleted = ' => 0));
	...
	private function validation_rules_insert(){
		...
		$this->form_validation->set_rules('mail',
									      ...
									      'required|valid_mail|is_unique[admin_user.mail]',

										  array('required' => 'Favor preencher o campo e-mail',
										  		'valid_mail' => 'Favor preencher o campo e-mail corretamente',
										  		'is_unique' => 'O e-mail %s já existe em nosso sistema'));
		...
		if($this->is_post()){
			...
			try{
				...
				if($this->form_validation->run() == TRUE){
					...
					$this->Default_model->update(array('name' => $this->input->post('name'),
												       'date_last_update' => date('Y-m-d H:i:s')),
	...
	private function validation_rules_edit($old_object){
		...
		if($old_object->mail != $this->input->post('mail')){
			$this->form_validation->set_rules('mail', $this->input->post('mail'), 'is_unique[admin_user.mail]', array('is_unique' => 'O e-mail %s já existe em nosso sistema'));
		}
	...
	public function sort_items_grid(){
		...
		if($this->is_post()){
			...
			foreach($ids as $id){
				...
				$this->Default_model->update(array('order' => $order),
										       array('id' => $id));
