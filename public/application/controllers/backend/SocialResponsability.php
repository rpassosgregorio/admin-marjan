...
class SocialResponsability extends MY_Controller_CMS {

	public $folder_view = 'social_responsability';
	public $title_module = 'Responsabilidade Social';
	public $controller_module = 'socialResponsability';
	...
	public function __construct(){
		...
		$this->table_module = $this->SocialResponsability_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::socialResponsability;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->SocialResponsability_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->SocialResponsability_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->SocialResponsability_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			$this->upload_file('social_responsability', $object->id);
	...
	public function upload_file($folder = 'social_responsability', $id = ''){

		if($folder != 'social_responsability') {
			$folder = 'social_responsability/'.$folder;
		}

		if (empty($id)) {

			$item = $this->SocialResponsability_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->SocialResponsability_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->SocialResponsability_model->update(array($file_field => $name_file), array('id' => $id));
