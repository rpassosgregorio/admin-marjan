...
class GeneralSettings extends MY_Controller_CMS {

	public $folder_view = 'general_settings';
	public $title_module = 'Configurações Gerais';
	public $controller_module = 'generalSettings';
	...
	public function __construct(){
		...
		$this->table_module = $this->GeneralSettings_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::generalSettings;
	...
	public function index(){
		...
		$this->view_data['object'] = $this->GeneralSettings_model->find_unique(array(), array(), '', array(), array(), 1, 1);
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->GeneralSettings_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->GeneralSettings_model->insert(array('active' 			=> 1,
				                                                      'deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));

				$object = $this->GeneralSettings_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));

			}

			foreach ($post as $key => $value) {
				$this->GeneralSettings_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
			}

			$this->upload_file('general_settings', $object->id);
	...
	public function upload_file($folder = 'general_settings', $id = ''){

		if($folder != 'general_settings') {
			$folder = 'general_settings/'.$folder;
		}

		if (empty($id)) {

			$item = $this->GeneralSettings_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->GeneralSettings_model->insert(array('deleted' 			=> 0,
				                                                 'order' 			=> 0,
				                                                 'inserted_by' 		=> $this->admin_logged->id,
				                                                 'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->GeneralSettings_model->update(array($file_field => $name_file), array('id' => $id));
