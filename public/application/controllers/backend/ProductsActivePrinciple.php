...
class ProductsActivePrinciple extends MY_Controller_CMS {

	public $folder_view = 'products_active_principle';
	public $title_module = 'Princípio ativo dos Produtos';
	public $controller_module = 'productsActivePrinciple';
	...
	public function __construct(){
		...
		$this->table_module = $this->ProductsActivePrinciple_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::productsActivePrinciple;
	...
	public function index($page = 1, $field_search = ''){
		...
		$this->view_data['list_objects'] = $this->ProductsActivePrinciple_model->find(array(), array('deleted = ' => 0), $field_search, array('id'), $order_field, $this->page_list, $this->count_pages_list);
		$this->view_data['total_record'] = $this->ProductsActivePrinciple_model->count(array(), array('deleted = ' => 0), $field_search, array('id'));
	...
	public function insert($page = 1, $field_search = ''){
		...
		$this->view_data['object'] = $this->ProductsActivePrinciple_model;
	...
	public function edit($page = 1, $id, $field_search = ''){
		...
		$this->view_data['page'] = $page;
		$this->view_data['field_search'] = $field_search;
		$this->view_data['object'] = $this->ProductsActivePrinciple_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
		...
		try{
			...
			if(!isset($_FILES) || empty($_FILES)){
				...
				if(!empty($id)){
					...
					$old_object = $this->ProductsActivePrinciple_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
					...
					if($return_upload['result'] == FALSE){
						...
						$this->ProductsActivePrinciple_model->update(array($file_field 		  => $new_name_file,
													   	   'extension' 		  => $extension,
												       	   'date_last_update' => date('Y-m-d H:i:s')),
								 			     	 array('id' => $id));

						$new_object = $this->ProductsActivePrinciple_model->find_unique(array(), array('id = ' => $id, 'deleted = ' => 0));
	...
	public function delete_item(){
		...
		$object = $this->ProductsActivePrinciple_model->find_unique(array(), array('id = ' => $this->input->post('id'), 'deleted = ' => 0));
		...
		if($this->is_post()){
			...
			try{
				...
				if($this->is_post()){
					...
					$this->ProductsActivePrinciple_model->update(array('active' => $active,
												       'date_last_update' => date('Y-m-d H:i:s')),
	...
	public function delete(){
		...
		try{
			...
			if(!empty($this->input->post('ids'))){
				...
				foreach($ids as $id){
					...
					$this->ProductsActivePrinciple_model->update(array('deleted' => 1,
												       'date_last_update' => date('Y-m-d H:i:s')),
		...
		echo json_encode(array("result" => $result, "msg" => $result == 'true' ? $feedback_success_message : $feedback_error_message, "total" => $this->ProductsActivePrinciple_model->count(array(), array('deleted = ' => 0), array(), array('id'))));
		...
		if($this->is_post()){
			...
			try{
				...
				if($this->form_validation->run() == TRUE){
					...
					$last_id = $this->ProductsActivePrinciple_model->insert(array('title'			 	=> $this->security->xss_clean($this->input->post('title')),
					                                                              'milligram_value'		=> $this->security->xss_clean($this->input->post('milligram_value')),
					                                                              'unit'			 	=> $this->security->xss_clean($this->input->post('unit')),
																  		    	  'inserted_by' 		=> $this->admin_logged->id));


					$new_object = $this->ProductsActivePrinciple_model->find_unique(array(), array('id = ' => $last_id, 'deleted = ' => 0));
		...
		if($this->is_post()){
			...
			try{
				...
				if($this->form_validation->run() == TRUE){
					...
					$this->ProductsActivePrinciple_model->update(array('title' 				=> $this->security->xss_clean($this->input->post('title')),
					                                                   'milligram_value'	=> $this->security->xss_clean($this->input->post('milligram_value')),
					                                                   'unit'			 	=> $this->security->xss_clean($this->input->post('unit')),
												       			 	   'date_last_update' 	=> date('Y-m-d H:i:s')),
	...
	public function sort_items_grid(){
		...
		if($this->is_post()){
			...
			foreach($ids as $id){
				...
				$this->ProductsActivePrinciple_model->update(array('order' => $order),
										       array('id' => $id));
