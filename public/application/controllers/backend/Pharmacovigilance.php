...
class Pharmacovigilance extends MY_Controller_CMS {

	public $folder_view = 'pharmacovigilance';
	public $title_module = 'Farmacovigilância';
	public $controller_module = 'pharmacovigilance';
	...
	public function __construct(){
		...
		$this->table_module = $this->Pharmacovigilance_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::pharmacovigilance;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->Pharmacovigilance_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->Pharmacovigilance_model->insert(array('active' 			=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->Pharmacovigilance_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			$this->upload_file('pharmacovigilance', $object->id);
	...
	public function upload_file($folder = 'pharmacovigilance', $id = ''){

		if($folder != 'pharmacovigilance') {
			$folder = 'pharmacovigilance/'.$folder;
		}

		if (empty($id)) {

			$item = $this->Pharmacovigilance_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->Pharmacovigilance_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->Pharmacovigilance_model->update(array($file_field => $name_file), array('id' => $id));
