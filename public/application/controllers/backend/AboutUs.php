...
class AboutUs extends MY_Controller_CMS {

	public $folder_view = 'about_us';
	public $title_module = 'Sobre Nós';
	public $controller_module = 'aboutus';
	...
	public function __construct(){
		...
		$this->table_module = $this->AboutUs_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::aboutUs;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->AboutUs_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->AboutUs_model->insert(array('active' 			=> 1,
				                                              'deleted' 		=> 0,
				                                              'order' 			=> 0,
				                                              'inserted_by' 	=> $this->admin_logged->id,
				                                              'date_insertion' 	=> date('Y-m-d H:i:s')));

				$object = $this->AboutUs_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key)){
					$this->AboutUs_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}

			}

			$this->upload_file('about', $object->id);
	...
	public function upload_file($folder = 'about', $id = ''){
		...
		if (empty($id)) {
			...
			$item = $this->AboutUs_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->AboutUs_model->insert(array('deleted' 			=> 0,
				                                         'order' 			=> 0,
				                                         'inserted_by' 		=> $this->admin_logged->id,
				                                         'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->AboutUs_model->update(array($file_field => $name_file), array('id' => $id));
