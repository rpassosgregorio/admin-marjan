...
class ContactUs extends MY_Controller_CMS {

	public $folder_view = 'contact_us';
	public $title_module = 'Fale Conosco';
	public $controller_module = 'contactUs';
	...
	public function __construct(){
		...
		$this->table_module = $this->ContactUs_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::contactUs;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->ContactUs_model->find_unique(array(), array(), '', array(), array(), 1, 1);
			...
			$this->upload_file('contact_us', $object->id);
	...
	public function upload_file($folder = 'contact_us', $id = ''){

		if($folder != 'contact_us') {
			$folder = 'contact_us/'.$folder;
		}

		if (empty($id)) {

			$item = $this->ContactUs_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->ContactUs_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->ContactUs_model->update(array($file_field => $name_file), array('id' => $id));
