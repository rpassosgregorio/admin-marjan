...
class AboutUsNewBusiness extends MY_Controller_CMS {

	public $folder_view = 'about_us_new_business';
	public $title_module = 'Novos Negócios';
	public $controller_module = 'aboutUsNewBusiness';
	...
	public function __construct(){
		...
		$this->table_module = $this->AboutUsNewBusiness_model->table_default;

		if(empty($this->menu_allowed_admin)){
			$this->redirect_login();
		}

		$this->id_subomdule = AdminSubmodule_model::aboutUsNewBusiness;
	...
	public function save(){
		...
		if($this->is_post()){
			...
			$object = $this->AboutUsNewBusiness_model->find_unique(array(), array(), '', array(), array(), 1, 1);

			if (!$object) {

				$last_id = $this->AboutUsNewBusiness_model->insert(array('active' 		=> 1,
				                                                           'deleted' 		=> 0,
				                                                           'order' 			=> 0,
				                                                           'inserted_by' 	=> $this->admin_logged->id,
				                                                           'date_insertion' => date('Y-m-d H:i:s')));

				$object = $this->AboutUsNewBusiness_model->find_unique(array(), array('id = ' => $last_id, 'active = ' => 1, 'deleted = ' => 0));
			...
			foreach ($post as $key => $value) {
				...
				if(!empty($key) && !empty($value)){
					$this->AboutUsNewBusiness_model->update(array($key => $this->nl2br_str($this->security->xss_clean($value))), array('id' => $object->id));
				}
	...
	public function upload_file($folder = 'new_business', $id = ''){
		...
		if (empty($id)) {
			...
			$item = $this->AboutUsNewBusiness_model->find_unique(array('id'), array(), '', array(), array(), 1, 1);

			if (!$item) {
				$id = $this->AboutUsNewBusiness_model->insert(array('deleted' 		=> 0,
				                                                      'order' 			=> 0,
				                                                      'inserted_by' 	=> $this->admin_logged->id,
				                                                      'date_insertion' 	=> date('Y-m-d H:i:s')));
			}else{
		...
		if(isset($_FILES) && !empty($_FILES)){
			...
			foreach ($uploads as $upload) {
				...
				$this->AboutUsNewBusiness_model->update(array($file_field => $name_file), array('id' => $id));
