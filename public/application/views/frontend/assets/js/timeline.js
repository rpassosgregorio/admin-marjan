...
var timeline_fns = {
	...
	Initialize: function(tl_wdt, tl_hgt){
		this.loadTimeline(tl_wdt, tl_hgt);
		this.LoadResizeEvents();
	},
	
	loadTimeline: function(_width, _height){
		...
		if(_width){
			...
			wdt = $(".know-more-about-mj-items").width();
		}
		...
		var TlUpperAreaHeight = hgt * 53 / 100;
		var TlLowerAreaHeight = hgt * 43 / 100;
		var TlDraggerHeight = (hgt * 4 / 100) + 1;
		
		$("#mytimeline").timelineSlider({

			// SIZING OPTIONS
			timelineWidth: wdt,					// TIMELINE GLOBAL WIDTH, value in pixels OR 'auto' (WITH QUOTES) FOR FULLWIDTH
			timelineHeight: hgt,				// TIMELINE GLOBAL HEIGHT, value in pixels
			upperAreaHeight: TlUpperAreaHeight,				// UPPER AREA (IMAGES) HEIGHT, value in pixels - 265
			lowerAreaHeight: TlLowerAreaHeight,				// LOWER AREA (MILESTONES) HEIGHT, value in pixels - 215
			draggerHeight: TlDraggerHeight,					// DRAGGER (SCROLLBAR) HEIGHT, value in pixels
		...
		$(".marks div").on("click", function(event){
			var year = event.currentTarget.dataset.label;
			$("#mytimeline .milestones .date").removeClass("active");
			$("#mytimeline .milestones .date._" + year).addClass("active");
		});
	},
	...
	LoadResizeEvents: function(){
		$(window).on("resize", (function(){			
			...
			var wdt = $(".know-more-about-mj-items").width();
			$("#mytimeline").width(wdt);
