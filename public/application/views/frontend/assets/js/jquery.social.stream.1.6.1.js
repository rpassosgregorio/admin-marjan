...
(function($){
	...
	$.extend(SocialStreamObject.prototype, {
		...
		create: function(el, options) {
			...
			this.defaults = {
				...
				dates: 'min,mins,hr,hrs,dia,dias,semana,semanas,mês,meses,ano,anos',
				transition: '0.8s',
				cache: true,
				container: 'dcsns',
				cstream: 'stream',
	...
	function nicetime(a,out,dates){
        var d = Math.round((+new Date - a) / 1000), fuzzy = '', d = d < 0 ? 0 : d,dates = dates == null ? 'min,mins,hr,hrs,dia,dias,semana,semanas,mês,meses,ano,anos' : dates, trans = dates.split(','), n = trans[1];
        if(out == 1) {
            return d;
        } else if(out == 0) {
